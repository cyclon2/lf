...
Theorem app_assoc : forall A (l m n:list A),
  l ++ m ++ n = (l ++ m) ++ n.
Proof.
intros a l m n. induction l as [| s l' IHl'].
* simpl. reflexivity.
* simpl. rewrite -> IHl'.  reflexivity.
Qed.
...
Proof.
intros a l m. induction l as [| s l' IHl'].
* simpl. reflexivity.
* simpl. rewrite -> IHl'.  reflexivity.
Qed.
(** [] *)
...
filter (fun n => 7 <=? n) (filter evenb l).
...
(** **** Exercise: 3 stars, standard (map_rev)  
    ...
    auxiliary lemma. *)
(* map f (rev l ++ [x0]) = map f (rev l) ++ [f x0] *)

Lemma map_combine : forall (X Y : Type) (a: X) (f : X -> Y) (l : list X),
  map f (l ++ [a]) = map f l ++ [f a].
Proof.
intros x y a f l. induction l.
* simpl. reflexivity.
* simpl. rewrite -> IHl. reflexivity.
Qed.
...
* simpl. reflexivity.
* simpl. rewrite <- IHl. rewrite -> map_combine. reflexivity.
Qed.
(** [] *)
...
Proof. intros x y z f p. induction p. reflexivity. Qed.
(** [] *)
